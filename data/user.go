...
func (user *User) CreateSession() (session Session, err error) {
	statement := "insert into sessions(uuid,email,user_id,created_at) values($1,$2,$3,$4) returning id,uuid, email," +
		"user_id,created_at"
...
func (user *User) Create() (err error) {
	...
	defer stmt.Close()
	err = stmt.QueryRow(createUUID(), user.Name, user.Email, encrypt(user.Password), time.Now()).Scan(&user.Id,
		&user.Uuid, &user.CreatedAt)
	return
}
func (session *Session) DeleteByUUID() (err error) {
	statement := "delete from sessions where uuid = $1"
	smt, err := Db.Prepare(statement)
	if err != nil {
		return err
	}
	defer smt.Close()
	_, err = smt.Exec(session.Uuid)
	return err
}
